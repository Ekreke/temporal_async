...
//
//go:embed examples/dead_loop.json
var loopJson string

// 执行入口： go test -v -run "^TestGenericWorkflow$"
func TestGenericWorkflow(t *testing.T) {
	...
	//	NodeName:       "root",
	//	Data:           []map[string]interface{}{initData},
	...
	//		Label:      "",
	//	},
	//}, options, GenericWorkflowWithMaxStep, sampleWorkflowJSON, initData, int64(0))
	//if err != nil {
	//	t.Error(err)
	//}

	//wkFlowId := we.GetID()
	//wkFlowRunId := we.GetRunID()
	//
	//// 发送开始信号，用于触发start节点
	//println("开始异步发送信号")
	//// feature 注意这里的开始节点信号名称
	//err = c.SignalWorkflow(context.Background(), wkFlowId, wkFlowRunId, "root", &notify.SignalData{
	//	ActivityExecId: "aty_1",
	//	NodeName:       "root",
	//	Data:           []map[string]interface{}{initData},
	//	Metadata: notify.SignalMetadata{
	...
	//})
	//if err != nil {
	//	t.Errorf("Unable to signal workflow：%s", err.Error())
	//}
	//println("初始异步发送信号完成")
}
