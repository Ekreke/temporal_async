...
func TestLoadPbDescriptor(t *testing.T) {
	r, err := convert.loadPbDescriptor(context.Background(), "https://github.acme.red/mapper/idl/blob/main/proto/mapper/taskargs/v1/args.proto", "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")
	if err != nil {
		t.Fatal(err)
	}
	fr, err := convert.findMessageDescriptor(r, "ServiceProbeTaskData")
	if err != nil {
		t.Fatal(err)
	}
...
func TestConvJsonToPB(t *testing.T) {
	...
	aa, err := convert.JSONToPB(context.Background(), "https://github.acme.red/mapper/idl/blob/main/proto/mapper/taskargs/v1/args.proto", "ServiceProbeTaskData", taskData)
	if err != nil {
		t.Fatal(err)
	}
	println(aa.String())
